...
public class Rotate {
    ...
    public static int Up_Down_Reverse = 0;
    public static int Left_Right_Reverse = 1;
    ...
    public static File rotateImage(File file, int degree) throws Exception {
        if (degree == 90) {
            return rotateImage90(file);
        }
        if (degree == 180) {
            return rotateImage180(file);
        }
        if (degree == 270) {
            return rotateImage270(file);
        }
        return null;
    }
    ...
    public static Image rotateImage(Image Image, int degree) {
        ...
        if (degree == 270) {
            ...
            Graphics2D graphics2d;
            (graphics2d
                    = (img = new BufferedImage(h, w, type)).createGraphics()).setRenderingHint(RenderingHints.KEY_INTERPOLATION, RenderingHints.VALUE_INTERPOLATION_BILINEAR);
            graphics2d.rotate(Math.toRadians(360 - degree), w / 2, h / 2 + (w - h) / 2);
